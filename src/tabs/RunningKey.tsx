...
import { runningKeyDecode, runningKeyEncode } from "../functions/Functions"

interface RunningKeyProps {
    onResult: (result: any) => void
}

const RunningKey = (props: RunningKeyProps) => {
                ...
                <>
                    ...
                    <View style={{ flexDirection: 'row' }}>
                        ...
                        <TouchableOpacity style={{ flex: 1, padding: 8, backgroundColor: Color.primary }} onPress={() => {
                            const result = runningKeyEncode(plainText, key)
                            props.onResult([
                                {
                                    label: 'Bản rõ',
                                    value: plainText
            ...
            {mode === 'decode' &&
                <>
                    ...
                    <View style={{ flexDirection: 'row' }}>
                        ...
                        <TouchableOpacity style={{ flex: 1, padding: 8, backgroundColor: Color.primary }} onPress={() => {
                            const result = runningKeyDecode(cipherText, decodeKey)
                            props.onResult([
                                {
                                    label: 'Bản mã',
                                    value: cipherText
